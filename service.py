...
from algo_sorter import AlgoSorter
...
def create_words_left_results(guesses: list[str], target: str) -> WordsLeftResults:
    game = WordleGame(target, guesses)
    wfilter = WordsLeftFilter(game)
    return WordsLeftResults(wfilter)

def create_middling_letters(guesses: list[str], target: str) -> MiddlingLetters:
    words_left_results = create_words_left_results(guesses, target)
    return MiddlingLetters(words_left_results)

def create_middling_words(guesses: list[str], target: str) -> MiddlingWords:
    middling_letters = create_middling_letters(guesses, target)
    return MiddlingWords(middling_letters)

def create_algo(guesses: list[str], target: str) -> Algo:
    middling_words = create_middling_words(guesses, target)
    return Algo(middling_words)

def create_sorted_algo(guesses: list[str], target: str) -> AlgoSorter:
    algo = create_algo(guesses, target)
    return AlgoSorter(algo)

def get_targets_left_for_api(guesses: list[str], target: str) -> list[str]:
    results = create_words_left_results(guesses, target)
    return results.targets_left

def get_middling_letters_for_api(guesses: list[str], target: str) -> dict[str, int]:
    middling_letters = create_middling_letters(guesses, target)
    return middling_letters.letters_for_api

def get_middling_words_for_api(guesses: list[str], target: str) -> list[str]:
    middling_words = create_middling_words(guesses, target)
    return middling_words.words_for_algo

def get_sorted_algo_results_for_api(guesses: list[str], target: str) -> list[tuple[str, int]]:
    sorted_algo = create_sorted_algo(guesses, target)
    return sorted_algo.sorted_algo_results

def get_best_guess_for_api(guesses: list[str], target: str) -> tuple[str, int]:
    sorted_algo = create_sorted_algo(guesses, target)
    return sorted_algo.best_guess
